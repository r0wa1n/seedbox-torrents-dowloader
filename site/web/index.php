    ...
    <table class="table table-striped">
        ...
        foreach ($filesDetails as $fileDetail) {
            if ($fileDetail['file'] != 'recycle_bin') {
                ...
                echo '    <td style="word-break: break-all; line-height: 34px;">' . ($downloaded ? '<span class="glyphicon glyphicon-ok"></span>&nbsp;' : '') . $fileDetail['file'] . ' (<span class="italic">' . octetsToSize($fileSize) . '</span>)</td>';
                echo '    <td>';
                // 2 cases
                ...
                if (file_exists(TEMP_DIR . SEEDBOX_NAME . '/' . $fileDetail['file'])) {
                    ...
                    $percentNow = 100 * $size / $fileSize;
                    echo '<div class="progress-bar downloading" role="progressbar" aria-valuenow="' . $size . '" aria-valuemin="0" aria-valuemax="' . $fileSize . '" style="width: ' . $percentNow . '%" file="' . $fileNameEncoded . '"><span class="glyphicon glyphicon-transfer">&nbsp;' . octetsToSize($size) . '</span></div>';
                    echo '</div>';
                } else if ($downloaded) {
                    ...
                    echo '<button type="button" class="download btn btn-small btn-success" file="' . $fileNameEncoded . '"><span class="glyphicon glyphicon-save">&nbsp;Download</span></button>';
                }
                echo '    </td>';
                echo '</tr>';
            }
